...
def _hparams(algorithm, dataset, random_state):
    ...
    #----- TTA hparams --------
    hparams["lr_coef"]= (1., 1.) 
    hparams["init_threshold"] = (.90, .90)
    hparams["trade_off"] = (1., 1.)
    hparams["inc_rate"] = (.01, .01)
    #-------------------------
    ...
    hparams["backbone"] = ("resnet50", "resnet50")   #B_16
    #hparams["backbone"] = ("ViT-B16", "ViT-B16")
    hparams["embed_size"] = (128,128) 
    hparams["freeze_bn"] = (False, False) #true, true
    hparams["pretrained"] = (True, True)  # only for ResNet
    hparams["device"] = ("cuda:1", "cuda:1")
    if dataset not in SMALL_IMAGES:
        ...
        if dataset == "DomainNet":
            ...
            hparams["batch_size"] = (32, int(2 ** random_state.uniform(3, 5.5))) #16 
        if algorithm == "ARM":
